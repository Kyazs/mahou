...
- Dispatch **one verifier per issue**. Issues are independent -- batch multiple
  ...
  place parallel subagents are safe in review: each is read-only and they
  don't share working-tree state.
- Use `subagent_type: "general"` -- not `explore`. Verification needs to
  ...
  approach the other read-only agents use.
